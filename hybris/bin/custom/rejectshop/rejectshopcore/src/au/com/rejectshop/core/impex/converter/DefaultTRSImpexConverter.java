 ...
 *
 */
package au.com.rejectshop.core.impex.converter;
...
public class DefaultTRSImpexConverter extends DefaultImpexConverter
{
	...
	{
		final String[] attributes = row.get(0).split(";");
		final String code = attributes[0] + attributes[1];
		final String updatedValue = code + ";" + attributes[0] + ";" + attributes[1] + ";" + attributes[2];
		final Map<Integer, String> updatedRow = new HashMap<Integer, String>();

		if (row.keySet().iterator().hasNext() == true)
		{
			updatedRow.put(row.keySet().iterator().next(), updatedValue);
		}

		String result = EMPTY_STRING;
		if (!MapUtils.isEmpty(updatedRow))
		{
			...
			while (idx > -1)
			{
				...
				processRow(updatedRow, sequenceId, builder, copyIdx, idx, endIdx);
				copyIdx = endIdx + 1;
