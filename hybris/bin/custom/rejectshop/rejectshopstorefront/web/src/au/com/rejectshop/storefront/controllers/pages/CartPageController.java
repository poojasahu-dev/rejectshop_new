...
public class CartPageController extends AbstractCartPageController
{
	...
	@ResponseBody
	public String removeCartItem(@RequestParam(value = "entryNumber") final long entryNumber,
			@RequestParam(value = "quantity", defaultValue = "0") final Long quantity, final Model model)
			throws CMSItemNotFoundException
	{
		if (getCartFacade().hasEntries())
		{
			try
			{
				...
				getCartFacade().updateCartEntry(entryNumber, quantity);
				prepareDataForPage(model);
			}
			catch (final CommerceCartModificationException ex)
			{
