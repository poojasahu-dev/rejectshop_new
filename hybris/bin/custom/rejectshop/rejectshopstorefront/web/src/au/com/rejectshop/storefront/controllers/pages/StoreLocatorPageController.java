...
@Controller
@Scope("tenant")
@RequestMapping(value = {"/store-finder","/storelocator"})
public class StoreLocatorPageController extends AbstractSearchPageController
{
	...
	{
		LOG.warn("StoreLocatorPageController googleAPI KEY ----"+configurationService.getConfiguration().getString(GOOGLE_API_VERSION));
	...
	public String getGoogleApiKey(final HttpServletRequest request)
	{
		...
		LOG.warn("StoreLocatorPageController googleAPI KEY ----"+googleApiKey);
	...
	public String getStoreFinderPage(final Model model, final HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		model.addAttribute("pageType", PageType.STOREFINDER.name());
		LOG.warn("StoreLocatorPageController.getStoreFinderPage");
		return getViewForPage(model);
	}


	@RequestMapping(value = "/locator", method = RequestMethod.GET)
	public String getStoreLocatorPage(final Model model, final HttpServletRequest request) throws CMSItemNotFoundException
	{
		return getStoreFinderPage(model, request);
	}
			...
			final Model model, final BindingResult bindingResult) throws CMSItemNotFoundException
	{
		...
		LOG.info("StoreLocatorPageController.findStores");
		if (latitude != null && longitude != null)
		{
			LOG.warn("StoreLocatorPageController.findStores latitude"+latitude+"---long--"+longitude);
		...
		{
			LOG.warn("StoreLocatorPageController.findStores search query---"+sanitizedSearchQuery);
			setUpSearchResultsForLocationQuery(sanitizedSearchQuery, createPageableData(page, getStoreLocatorPageSize(), sortCode, showMode), model);
			LOG.info("setUpMetaData");
			setUpMetaData(sanitizedSearchQuery, model);
			LOG.info("setUpPageForms");
			setUpPageForms(model);
			LOG.info("setUpPageTitle");
			setUpPageTitle(sanitizedSearchQuery, model);

		}
		else
		{
			LOG.warn("StoreLocatorPageController.findStores add error message");
		...
		return ControllerConstants.Views.Pages.StoreFinder.StoreFinderSearchResultsPage;
	}
	...
	{
		LOG.info("inside setupError Message "+searchResult.getResults().isEmpty());
	...
	protected void setUpSearchResultsForPosition(final GeoPoint geoPoint, final PageableData pageableData, final Model model)
	{
		...
		LOG.info("StoreLocatorPageController.setupSearchResultsForPosition "+searchResult);
	...
	{
		// Run the location search & populate the model
		//LOG.info("Do Store Location Search");
