 ...
 *
 */
package au.com.rejectshop.storefront.tags;
...
import au.com.rejectshop.storefront.util.FinancialReportYearComparator;
...
public class Functions
{
	...
	public static String encodeUrl(final String url)
	{
		...
		{
			LOG.error("Problem while encoding URL", e);
			return url;
		}
	}
	...
	/**
	 * Utility method to format size of a file in MBs/KBs/bytes for display
	 * 
	 * @param numberOfBytes
	 *           size of file in bytes
	 * @return formatted string with size of the file
	 */
	public static String formatFileSize(final Long numberOfBytes)
	{
		final int decimalPos = 1;
		final double sizeInBytes = numberOfBytes.doubleValue();
		...
		if (sizeInBytes < 1024)
		{
			formattedSize = formattedSize.append(fmt.format(numberOfBytes.longValue())).append(" bytes");
		}
		else
		{
			final double sizeInKBs = sizeInBytes / 1024;
			if (sizeInKBs < 1024)
			{
				formattedSize = formattedSize.append(fmt.format(sizeInKBs)).append(" KB");
			}
			else
			{
				final double sizeInMBs = sizeInBytes / (1024 * 1024);
				if (sizeInMBs < 1024)
				{
					formattedSize = formattedSize.append(fmt.format(sizeInMBs)).append(" MB");
				}
			}
		}
	...
	public static List<ReportMediasCMSComponentModel> sortListByFinancialYear(final List<ReportMediasCMSComponentModel> list)
	{
		final List<ReportMediasCMSComponentModel> modifiableReportMediaList = new ArrayList<ReportMediasCMSComponentModel>(list);
		Collections.sort(modifiableReportMediaList, new FinancialReportYearComparator());

		return modifiableReportMediaList;
	}
}
