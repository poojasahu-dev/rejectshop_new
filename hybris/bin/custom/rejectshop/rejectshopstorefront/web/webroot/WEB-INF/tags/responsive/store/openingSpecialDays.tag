...
<c:if test="${not empty openingSchedule}">
	{
			"Special":" <b>Opening Hours</b>" ,
				<c:forEach items="${openingSchedule.specialDayOpeningList}" var="specialDay">
			
					"Date" :"${specialDay.formattedDate}",
					"":"${specialDay.name}",
					"Time":"${specialDay.openingTime.formattedHour} - ${specialDay.closingTime.formattedHour} "
							<%-- <c:choose>
								<c:when test="${specialDay.closed}" >
										"<spring:theme code="storeDetails.table.opening.closed" />":"<spring:theme code="storeDetails.table.opening.closed" />",
								</c:when>
								<c:otherwise>
									"Time":"${specialDay.openingTime.formattedHour} - ${specialDay.closingTime.formattedHour} "
								</c:otherwise>
