...
<c:forEach items="${medias}" var="media">
	<c:choose>
		...
		<c:otherwise>
			<c:set var="imagerData">${imagerData},${media.url}</c:set>
		</c:otherwise>
	</c:choose>
	<c:if test="${empty altText}">
		<c:set var="altText" value="${media.altText}"/>
...
<div class="simple-responsive-banner-component">
	<c:choose>
		<c:when test="${empty encodedUrl || encodedUrl eq '#'}">
			<img class="js-responsive-image"  src='${imagerData}' alt='${altText}' title='${altText}' style="">
		</c:when>
		<c:otherwise>
		
				<a href="${encodedUrl}">
					<img class="js-responsive-image"  src='${imagerData}' title='${altText}' alt='${altText}' style="">
				</a>
		</c:otherwise>
	</c:choose>
</div>