...
<c:if test="${searchPageData.pagination.totalNumberOfResults > 0}">
	...
	<div class="pagination-bar ${(top)?"top":"bottom"}">
			<div class="${(top)? (searchPageData.pagination.numberOfPages == 1)? "col-xs-6 col-sm-6":"col-xs-12 col-sm-3":"col-xs-6"} product-count">
				<ycommerce:testId code="searchResults_productsFound_label">
			...
			<c:if test="${not empty searchPageData.sorts}">				
				<c:if test="${top}">
					<div class="${(searchPageData.pagination.numberOfPages == 1)?"col-xs-6 col-sm-6 no-paging":"col-xs-6 col-sm-4"}">
				...
				<c:if test="${searchPageData.pagination.numberOfPages == 1}"><div class="hide"></c:if>
				<div class="${(top)?"col-xs-12 col-sm-5":"col-xs-12 col-sm-6"}">
				...
				<c:if test="${searchPageData.pagination.numberOfPages == 1}"></div></c:if>
			</c:if>
