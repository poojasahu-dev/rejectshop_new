								...
								<c:forEach items="${child.links}" step="${component.wrapAfter}" var="childlink" varStatus="i">
									...
									<ul class="sub-navigation-list">
										...
										<c:if test="${fn:length(child.children)>0}">
												<c:forEach items="${child.children}" var="schild">
													<li class="auto has-sub js-enquire-has-sub"id="${schild.uid}" ><a href="${schild.links[0].url}" >${schild.title}</a></li>
													<div class="sub-navigation" style="margin-left: 80px;display:none;" id="${schild.uid}Link">
														<div class="sub-navigation-section">
															<c:forEach items="${schild.links}" step="${component.wrapAfter}" var="childlink" varStatus="i">
																<ul class="sub-navigation-list">
																	<c:forEach items="${schild.links}" var="childlink" begin="${i.index+1}" end="${i.index + component.wrapAfter - 1}">
																		<cms:component component="${childlink}" evaluateRestriction="true" element="li" />
																		<%--<li class="auto has-sub js-enquire-has-sub"><a>${childlink.name}</a></li> --%>
																	</c:forEach>
																</ul>
															</c:forEach>
