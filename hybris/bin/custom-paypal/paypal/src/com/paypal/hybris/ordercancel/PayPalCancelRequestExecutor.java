...
public class PayPalCancelRequestExecutor extends ImmediateCancelRequestExecutor
{
	...
	@Resource(name = "paypalPaymentIntegrationService")
	private PaypalPaymentIntegrationService paypalPaymentIntegrationService;

	/*
	 * (non-Javadoc)
	 *
	 * @see de.hybris.platform.ordercancel.impl.executors.ImmediateCancelRequestExecutor#processCancelRequest(de.hybris.
	 * platform .ordercancel.OrderCancelRequest, de.hybris.platform.ordercancel.model.OrderCancelRecordEntryModel)
	 */
	@Override
			...
			final OrderCancelRecordEntryModel cancelRequestRecordEntry) throws OrderCancelException
	{
		...
		{
			final List<PaymentTransactionModel> paymentTransactionList = orderCancelRequest.getOrder().getPaymentTransactions();
				...
				if (CollectionUtils.isNotEmpty(paymentTransactionEntryList))
				{
					for (final PaymentTransactionEntryModel paymentTransactionEntry : paymentTransactionEntryList)
					{
						if (PaymentTransactionType.AUTHORIZATION.equals(paymentTransactionEntry.getType()))
						{
							...
							final DoVoidResponseType res = paypalPaymentIntegrationService.doVoid(request);

							if (res.getAck() != AckCodeType.SUCCESS)
							{
								final String errorsMessage = StringUtils.join(PaypalStringUtils.getErrorMessagesList(res.getErrors()),
										"; ");
