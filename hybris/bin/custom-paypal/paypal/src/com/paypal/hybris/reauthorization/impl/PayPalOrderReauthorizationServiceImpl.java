...
public class PayPalOrderReauthorizationServiceImpl implements PayPalOrderReauthorizationService
{
	...
	public OrderModel reauthorize(final OrderModel orderModel, final String amount)
	{
		...
		final DoReauthorizationResponseType doReauthorizationResponseType = getPaypalPaymentIntegrationService()
		...
		if (AckCodeType.SUCCESS.equals(doReauthorizationResponseType.getAck()))
		{
					...
					&& PendingStatusCodeType.AUTHORIZATION != doReauthorizationResponseType.getAuthorizationInfo().getPendingReason())
			{
				reauthTransactionEntry = createTransactionEntry(orderModel.getPaymentTransactions().get(0),
						PaymentTransactionType.REAUTHORIZATION, PaymentStatus.PENDING.name(),
						doReauthorizationResponseType.getAuthorizationInfo().getPendingReason().name(),
						doReauthorizationResponseType.getAuthorizationID(), currencyID, amount,
						PaypalStringUtils.getDateFromResponse(doReauthorizationResponseType.getTimestamp()));
			}
			else
			{
				reauthTransactionEntry = createTransactionEntry(orderModel.getPaymentTransactions().get(0),
						...
						amount, PaypalStringUtils.getDateFromResponse(doReauthorizationResponseType.getTimestamp()));
			}
		}
		else
		{
