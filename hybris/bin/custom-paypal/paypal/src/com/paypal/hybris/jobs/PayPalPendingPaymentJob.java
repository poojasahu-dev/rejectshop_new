...
public class PayPalPendingPaymentJob extends AbstractJobPerformable<CronJobModel>
{
	...
	public PerformResult perform(final CronJobModel paramT)
	{
		...
		{
			final GetTransactionDetailsResultData transactionDetails = getTransactionDetailsService()
					.getTransactionDetails(paymentTransactionEntryModel.getRequestId());
			if (AckCodeType.SUCCESS.name().equalsIgnoreCase(transactionDetails.getAck()))
			{
				if (!paymentTransactionEntryModel.getTransactionStatus()
						.equalsIgnoreCase(transactionDetails.getPaymentStatus().name()))
				{
