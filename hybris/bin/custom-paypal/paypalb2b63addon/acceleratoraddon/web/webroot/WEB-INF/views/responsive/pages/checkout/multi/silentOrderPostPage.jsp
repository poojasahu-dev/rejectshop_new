...
<spring:eval expression="@configurationService.configuration.getProperty('paypal.seller.email')" var="merchantId"/>
		...
		<multiCheckout:checkoutSteps checkoutSteps="${checkoutSteps}" progressBarId="${progressBarId}">
			<jsp:body>
                <c:if test="${not empty paymentFormUrl}">
					...
					<a id="silentOrderSubmitButton" class="btn btn-primary btn-block submit_silentOrderPostForm checkout-next">
						<spring:theme code="checkout.multi.paymentMethod.continue"/>
					</a>

					<div id="mark-paypal-button-container" class="checkout-next" align="center">
						<div>
							<input type="checkbox" id="isPayPalReplenishment" onclick="checkReplenishment()"/>
							<label>Check for replenishment</label>
						</div>
					</div>
