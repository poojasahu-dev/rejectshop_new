...
@RestController
public class PayPalCreatePaymentController extends AbstractCheckoutController {
    ...
    private static final Logger LOG = Logger.getLogger(PayPalCreatePaymentController.class.getName());
    ...
    public TokenResponse paymentEC(final Model model) {
        ...
        if (!configurationService.getConfiguration().getBoolean(PaypalConstants.PAYPAL_GUEST_REDIRECT)) {
            return prepareExpressCheckout(model, "/paypal/checkout/hop/response/?shortcut=true", "/cart",
                    PaypalConstants.CREDIT_CARD, getSolutionType(), true);
        } else {
            return prepareExpressCheckout(model, "/paypal/checkout/hop/response/?shortcut=false", "/cart",
                    PaypalConstants.CREDIT_CARD, getSolutionType(), true);
        }
    }
    ...
    private void handleErrors(final AbstractResultData resultData) {
        ...
        getSessionService().setAttribute(Paypal66addonControllerConstants.PAY_PAL_ERRORS_DETAILS, errorCodeToMsgMap);
    ...
    private String getPayPalErrorMessage() {
        Map<String, String> payPalErrors = (Map<String, String>) getSessionService().getAttribute(Paypal66addonControllerConstants.PAY_PAL_ERRORS_DETAILS);
