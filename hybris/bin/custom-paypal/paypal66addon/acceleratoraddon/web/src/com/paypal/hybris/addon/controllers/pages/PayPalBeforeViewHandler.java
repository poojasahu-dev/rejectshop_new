...
import com.paypal.hybris.addon.constants.Paypal66addonConstants;
import com.paypal.hybris.addon.controllers.Paypal66addonControllerConstants;
...
public class PayPalBeforeViewHandler implements BeforeViewHandlerAdaptee
{
                             ...
                             final String viewName) throws Exception
	{
		...
		{
			return Paypal66addonControllerConstants.Views.Pages.MultiStepCheckout.SilentOrderPostPage;
		}
		else if (B2C_CHECKOUT_SUMMARY_PAGE.equals(viewName))
		{
			final String repeatRedirectUrl = getSessionService().getAttribute((Paypal66addonConstants.PAY_PAL_REPEAT_REDIRECT_URL));
			if (StringUtils.isNotBlank(repeatRedirectUrl))
			{
				getSessionService().removeAttribute(Paypal66addonConstants.PAY_PAL_REPEAT_REDIRECT_URL);
				return "redirect:" + repeatRedirectUrl;
			}
			else
			{
				return Paypal66addonControllerConstants.Views.Pages.MultiStepCheckout.CheckoutSummaryPage;
			}
		}
		else if (B2C_CHECKOUT_CONFIRMATION_PAGE.equals(viewName))
		{
			return Paypal66addonControllerConstants.Views.Pages.Checkout.CheckoutConfirmationPage;
		}
		else if (B2C_ADD_TO_CART_POPUP_PAGE.equals(viewName))
		{
			...
			return Paypal66addonControllerConstants.Views.Fragments.Cart.AddToCartPopup;
		}
		else if (isCheckoutPage(viewName) || isPayPalCheckoutRedirect(viewName, request))
		{
			...
			final String payPalHopUrl = getSessionService().getAttribute(Paypal66addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
			if (StringUtils.isNotBlank(payPalHopUrl))
			{
				getSessionService().removeAttribute(Paypal66addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
				return "redirect:" + payPalHopUrl;
			}
		}

		if (!StringUtils.contains(viewName, Paypal66addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage)
	...
	{
		return !Paypal66addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage.equals(viewName)
				&& StringUtils.contains(viewName, Paypal66addonControllerConstants.CHECKOUT_PAGE_VIEW_NAME_FRAGMENT);
	}
