...
public class PayPalReturnsController extends DefaultReturnsController
{
		...
		try
		{
			...
			for (ObjectValueContainer ovc : refundEntriesValueContainers)
			{
				...
				if (returnableOrderEntries.containsKey(orderEntry))
				{
					...
					if ((expectedQty > 0L) && (expectedQty <= SafeUnbox.toLong(returnableOrderEntries.get(orderEntry))) && (reason != null) &&
							(action != null))
					{
	...
	public TypedObject createRefundRequest()
	{
		...
		{
			OrderModel orderModel = getOrderModel();
			ReturnRequestModel returnRequest = getReturnService().createReturnRequest(orderModel);
			String rmaString = getReturnService().createRMA(returnRequest);
			returnRequest.setRMA(rmaString);

			applyRefunds(orderModel, returnRequest, this.refundDetailsList, false);

			TypedObject refundRequestObject = getCockpitTypeService().wrapItem(returnRequest);
		...
		{
			LOG.error("Failed to create refund request, message: " + e.getMessage(), e);
		}
