...
public class ReauthorizeRequestCreateWidgetRenderer extends AbstractCreateWidgetRenderer
{
			...
			throws InterruptedException
	{
		if ("onOK".equals(event.getName()))
		{
			try
			{
				...
				{
					final TypedObject typedObject = ((PayPalReauthorizationController) widget.getWidgetController())
	...
	private boolean isLastTransactionSuccessful(final TypedObject typedObject)
	{
		...
		if (typedObject != null)
		{
			...
			{
				if (PaymentTransactionType.REAUTHORIZATION.equals(lastTransactionEntry.getType())
