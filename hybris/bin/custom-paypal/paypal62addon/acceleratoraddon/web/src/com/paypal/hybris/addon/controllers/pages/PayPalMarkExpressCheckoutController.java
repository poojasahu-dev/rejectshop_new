...
@RestController
public class PayPalMarkExpressCheckoutController extends AbstractCheckoutController {
    ...
    private static final Logger LOG = Logger.getLogger(PayPalMarkExpressCheckoutController.class.getName());
    ...
    public TokenResponse markEC(final Model model) {
        ...
        if (!configurationService.getConfiguration().getBoolean(PaypalConstants.PAYPAL_GUEST_REDIRECT)) {
            return prepareMarkExpressCheckout(model, "/paypal/checkout/hop/response/?shortcut=true", "/cart",
                    PaypalConstants.CREDIT_CARD, getSolutionType(), true);
        } else {
            return prepareMarkExpressCheckout(model, "/paypal/checkout/hop/response/?shortcut=false", "/cart",
                    PaypalConstants.CREDIT_CARD, getSolutionType(), true);
        }
    }

    public TokenResponse prepareMarkExpressCheckout(final Model model, final String returnUrl, final String cancelUrl,
        ...
        if (!PaypalConstants.STATUS_SUCCESS.equalsIgnoreCase(resultData.getAck())) {
            ...
            if (hasShippingError(errorCodes)) {
                ...
                getSessionService().getCurrentSession().setAttribute("paypal.general.error.shippingAddress", model.asMap().get("accErrorMsgs"));
    ...
    private String getPayPalErrorMessage() {
        Map<String, String> payPalErrors = (Map<String, String>) getSessionService().getAttribute(Paypal62addonControllerConstants.PAY_PAL_ERRORS_DETAILS);
    ...
    private void handleErrors(final AbstractResultData resultData) {
        ...
        getSessionService().setAttribute(Paypal62addonControllerConstants.PAY_PAL_ERRORS_DETAILS, errorCodeToMsgMap);
