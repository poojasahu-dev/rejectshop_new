...
import com.paypal.hybris.addon.constants.Paypal62addonConstants;
import com.paypal.hybris.addon.controllers.Paypal62addonControllerConstants;
...
public class PayPalBeforeViewHandler implements BeforeViewHandlerAdaptee
{
			...
			final String viewName) throws Exception
	{
		...
		{
			return Paypal62addonControllerConstants.Views.Pages.MultiStepCheckout.SilentOrderPostPage;
		}
		else if (B2C_CHECKOUT_SUMMARY_PAGE.equals(viewName))
		{
			final String repeatRedirectUrl = getSessionService().getAttribute((Paypal62addonConstants.PAY_PAL_REPEAT_REDIRECT_URL));
			if (StringUtils.isNotBlank(repeatRedirectUrl))
			{
				getSessionService().removeAttribute(Paypal62addonConstants.PAY_PAL_REPEAT_REDIRECT_URL);
				return "redirect:" + repeatRedirectUrl;
			}
			else
			{
				return Paypal62addonControllerConstants.Views.Pages.MultiStepCheckout.CheckoutSummaryPage;
			}
		}
		else if (B2C_CHECKOUT_CONFIRMATION_PAGE.equals(viewName))
		{
			return Paypal62addonControllerConstants.Views.Pages.Checkout.CheckoutConfirmationPage;
		}
		else if (B2C_ADD_TO_CART_POPUP_PAGE.equals(viewName))
		{
			...
			return Paypal62addonControllerConstants.Views.Fragments.Cart.AddToCartPopup;
		}
		else if (isCheckoutPage(viewName) || isPayPalCheckoutRedirect(viewName, request))
		{
			...
			final String payPalHopUrl = getSessionService().getAttribute(Paypal62addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
			if (StringUtils.isNotBlank(payPalHopUrl))
			{
				getSessionService().removeAttribute(Paypal62addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
				return "redirect:" + payPalHopUrl;
			}
		}

		if (!StringUtils.contains(viewName, Paypal62addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage)
	...
	{
		return !Paypal62addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage.equals(viewName)
				&& StringUtils.contains(viewName, Paypal62addonControllerConstants.CHECKOUT_PAGE_VIEW_NAME_FRAGMENT);
	}
