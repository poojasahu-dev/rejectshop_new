...
public class PayPalBeforeViewHandler implements BeforeViewHandlerAdaptee
{
			...
			final String viewName) throws Exception
	{
		...
		{
			return Paypalb2b60addonControllerConstants.Views.Pages.MultiStepCheckout.SilentOrderPostPage;
		}
		else if (B2B_CHECKOUT_SUMMARY_PAGE.equals(viewName))
		{
			final String repeatRedirectUrl = getSessionService()
					.getAttribute((Paypalb2b60addonConstants.PAY_PAL_REPEAT_REDIRECT_URL));
			...
			{
				getSessionService().removeAttribute(Paypalb2b60addonConstants.PAY_PAL_REPEAT_REDIRECT_URL);
				return "redirect:" + repeatRedirectUrl;
			}
			else
			{
				return Paypalb2b60addonControllerConstants.Views.Pages.MultiStepCheckout.CheckoutSummaryPage;
			}
		}
		else if (B2B_CHECKOUT_CONFIRMATION_PAGE.equals(viewName))
		{
			return Paypalb2b60addonControllerConstants.Views.Pages.Checkout.CheckoutConfirmationPage;
		}
		else if (ADD_TO_CART_POPUP_PAGE.equals(viewName))
		{
			...
			return Paypalb2b60addonControllerConstants.Views.Fragments.Cart.AddToCartPopup;
		}
		else if (isCheckoutPage(viewName) || isPayPalCheckoutRedirect(viewName, request))
		{
			...
			final String payPalHopUrl = getSessionService().getAttribute(Paypalb2b60addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
			if (StringUtils.isNotBlank(payPalHopUrl))
			{
				getSessionService().removeAttribute(Paypalb2b60addonControllerConstants.PAY_PAL_HOP_URL_ATTR);
				return "redirect:" + payPalHopUrl;
			}
		}

		if (!StringUtils.contains(viewName, Paypalb2b60addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage)
	...
	{
		boolean result;
		result = !Paypalb2b60addonControllerConstants.Views.Pages.Checkout.CheckoutLoginPage.equals(viewName)
				&& StringUtils.contains(viewName, Paypalb2b60addonControllerConstants.CHECKOUT_PAGE_VIEW_NAME_FRAGMENT);
