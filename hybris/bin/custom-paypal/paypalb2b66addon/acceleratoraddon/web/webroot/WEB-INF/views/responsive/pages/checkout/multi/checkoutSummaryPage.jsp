...
<%@ taglib prefix="multi-checkout-paypal" tagdir="/WEB-INF/tags/addons/paypalb2b66addon/responsive/checkout/multi" %>

<spring:url value="/checkout/multi/summary/placeOrder" var="placeOrderUrl"/>
<spring:url value="/checkout/multi/termsAndConditions" var="getTermsAndConditionsUrl"/>
<spring:url value="/paypal/b2b/replenishment/error" var="replenishmentErrorUrl"/>
                ...
                <ycommerce:testId code="checkoutStepFour">
                    ...
                    <div class="place-order-form hidden-xs">
                        <form:form action="${placeOrderUrl}" id="placeOrderForm1" commandName="placeOrderForm">
                            ...
                            <c:choose>
                                <c:when test="${isPlaceOrderAvailable == true}">
                                ...
                                <c:otherwise>
                                    <button id="placeOrder" type="submit"
                                            class="btn btn-primary btn-block btn-place-order btn-block btn-lg"
                                            disabled="disabled">
                                        <spring:theme code="checkout.summary.placeOrder"/>
                                    </button>
                                </c:otherwise>
                            ...
                            <c:if test="${cartData.quoteData eq null}">
                                <c:choose>
                                    <c:when test="${isReplenishmentAvailable == true}">
                                    ...
                                    <c:otherwise>
                                        <button id="scheduleReplenishment" type="button"
                                                class="btn btn-default btn-block scheduleReplenishmentButton"
                                                disabled="disabled">
                                            <spring:theme code="checkout.summary.scheduleReplenishment"/>
                                        </button>
                                    </c:otherwise>
